...
// AddGold 增加金币
func (d *UserDAO) AddGold(ctx context.Context, id int64, amount int) error {
        if amount < 0 {
                amount = 0
        }
        _, err := d.db.ExecContext(ctx,
                `UPDATE users SET gold = gold + ?, updated_at = NOW() WHERE id = ?`, amount, id,
        )
        return err
}

// DeductGold 扣除金币
func (d *UserDAO) DeductGold(ctx context.Context, id int64, amount int) error {
        if amount < 0 {
                amount = 0
        }
        _, err := d.db.ExecContext(ctx,
                `UPDATE users SET gold = GREATEST(gold - ?, 0), updated_at = NOW() WHERE id = ?`, amount, id,
        )
        return err
}

// AddDiamonds 增加钻石
func (d *UserDAO) AddDiamonds(ctx context.Context, id int64, amount int) error {
        if amount < 0 {
                amount = 0
        }
        _, err := d.db.ExecContext(ctx,
                `UPDATE users SET diamond = diamond + ?, updated_at = NOW() WHERE id = ?`, amount, id,
        )
        return err
}

// DeductDiamonds 扣除钻石
func (d *UserDAO) DeductDiamonds(ctx context.Context, id int64, amount int) (bool, error) {
        if amount <= 0 {
                return false, nil
        }
        result, err := d.db.ExecContext(ctx,
                `UPDATE users SET diamond = GREATEST(diamond - ?, 0), updated_at = NOW() WHERE id = ? AND diamond >= ?`,
                amount, id, amount,
        )
        if err != nil {
                return false, err
...
// AddFood 增加粮食
func (d *UserDAO) AddFood(ctx context.Context, id int64, amount int) error {
        if amount < 0 {
                amount = 0
        }
        _, err := d.db.ExecContext(ctx,
                `UPDATE users SET food = COALESCE(food, 0) + ?, updated_at = NOW() WHERE id = ?`, amount, id,
        )
        return err
}

// DeductFood 扣除粮食
func (d *UserDAO) DeductFood(ctx context.Context, id int64, amount int) (bool, error) {
        if amount <= 0 {
                return false, nil
        }
        result, err := d.db.ExecContext(ctx,
                `UPDATE users SET food = GREATEST(COALESCE(food, 0) - ?, 0), updated_at = NOW() WHERE id = ? AND COALESCE(food, 0) >= ?`,
                amount, id, amount,
        )
        if err != nil {
                return false, err
...
// AddExp 增加经验（自动升级）
func (d *UserDAO) AddExp(ctx context.Context, id int64, exp int64) (int, error) {
        if exp <= 0 {
                return 0, nil
        }
        result, err := d.db.ExecContext(ctx,
                `UPDATE users SET experience = experience + ?, updated_at = NOW() WHERE id = ?`,
                exp, id,
        )
        if err != nil {
                return 0, err
...
// GetDiamondBalance 获取钻石余额
func (d *UserDAO) GetDiamondBalance(ctx context.Context, id int64) (int64, error) {
        var diamond int64
        err := d.db.QueryRowContext(ctx,
                `SELECT diamond FROM users WHERE id = ?`, id,
        ).Scan(&diamond)
        return diamond, err
}
