...
func main() {
	...
	// 区服系统
	serverDAO := dao.NewServerDAO(pkgmysql.DB)
	serverService := service.NewServerService(serverDAO)
	...
	engine := gin.New()
	router.Setup(engine, userHandler, internalHandler, serverHandler)
	...
	fmt.Println("   ── 区服接口 ──────────────────────")
	fmt.Println("   区服列表 → GET    /api/v1/servers")
	fmt.Println("   可进区服 → GET    /api/v1/servers/running")
	fmt.Println("   区服详情 → GET    /api/v1/servers/:id")
	fmt.Println("   选择区服 → POST   /api/v1/servers/select  [需要JWT]")
	fmt.Println("   我的区服 → GET    /api/v1/servers/my      [需要JWT]")
	fmt.Println("   ── 内部接口 ──────────────────────")
