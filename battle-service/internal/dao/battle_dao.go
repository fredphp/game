...
// HasClearedStage 检查玩家是否已通关指定关卡
func (d *BattleDAO) HasClearedStage(ctx context.Context, userID int64, stageID int) (bool, error) {
        var count int
        err := d.db.QueryRowContext(ctx,
                `SELECT COUNT(*) FROM battle_records WHERE attacker_id = ? AND stage_id = ? AND attacker_win = 1 AND type = 'pve'`,
                userID, stageID,
        ).Scan(&count)
        if err != nil {
                return false, fmt.Errorf("check stage clear: %w", err)
        }
        return count > 0, nil
}
