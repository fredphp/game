...
func (e *MarchEngine) InitiateMarch(ctx context.Context, userID, allianceID int64, sourceCityID, targetCityID int64,
        ...
        // 6. 扣除用户粮食
        if _, err := callUserInternal(ctx, "POST", "/internal/user/deduct-food", map[string]interface{}{
                "user_id": userID,
                "amount":  foodCost,
        }); err != nil {
                return nil, fmt.Errorf("deduct food failed: %w (need %d)", err, foodCost)
        }
...
func (e *MarchEngine) RecallMarch(ctx context.Context, userID int64, marchID string) error {
        ...
        if refundFood > 0 {
                if _, err := callUserInternal(ctx, "POST", "/internal/user/add-food", map[string]interface{}{
                        "user_id": userID,
                        "amount":  refundFood,
                }); err != nil {
                        log.Printf("❌ 返还粮食失败: user=%d amount=%d err=%v", userID, refundFood, err)
                }
        }
